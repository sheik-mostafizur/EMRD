...
(function ($) {
    ...
    function svg() {
      ...
      // Chart definition.
      var pie = d3.pie().padAngle(.001).value(function (d) {
        return d.ratio;
      }).sort(null)(data);

      // Chart's arc definition.
      var arc = d3.arc().padRadius(outerRadius).innerRadius(innerRadius).startAngle(function (d) {
      ...
      var labelArc = d3.arc().outerRadius(outerRadius - 10).innerRadius(innerRadius - 10).startAngle(function (d) {
        return d.startAngle + Math.PI / 2 - .2;
      }).endAngle(function (d) {
        return d.endAngle + Math.PI / 2 - .2;
      });
      ...
      if (window.matchMedia('(max-width: 767px)').matches) {
        ...
        svg = d3.select('#emrdRing').append('svg').
      ...
      var g = svg.selectAll('path').data(pie).enter().append('g').attr('id', function (d) {
      ...
      }).attr('dx', '-34px').attr('dy', '5px').attr('class', 'gtext').text(function (d) {
      ...
      function enableElement(outerRadius, delay) {
        return function (d) {
          ...
          attr('transform', function (dd) {
            return 'translate(' + labelArc.centroid(dd) + 10 + ') rotate(' + -rotate + ')';
          }).
          duration(1000);
